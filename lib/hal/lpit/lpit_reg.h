 ...
 * @details
 * Định nghĩa các thanh ghi và bit field của module LPIT.
 * LPIT là timer 32-bit dùng cho timing, PWM, và các ứng dụng low-power.
 * 
 * S32K144 có LPIT với:
 * - 4 kênh timer độc lập (Channel 0-3)
 * - 32-bit counter cho mỗi kênh
 * - Các chế độ: Periodic Counter, Dual 16-bit Periodic Counter
 ...
 * @version 1.0
 * 
 * @note    Tham khảo S32K1xx Reference Manual Chapter 43 (LPIT)
 * @warning Phải enable clock cho LPIT trước khi sử dụng
 * 
 * @par Change Log:
...
/** @brief Địa chỉ base của LPIT0 module */
#define LPIT0_BASE          (0x40037000UL)

/** @brief Số lượng kênh LPIT */
 ...
 * @details Thanh ghi điều khiển chung cho LPIT module
 */
typedef struct {
 ...
 * @details Thanh ghi trạng thái của các kênh timer
 */
typedef struct {
 ...
 * @details Thanh ghi enable interrupt cho các kênh
 */
typedef struct {
 ...
 * @details Thanh ghi để set interrupt enable (write-only)
 */
typedef struct {
 ...
 * @details Thanh ghi để clear interrupt enable (write-only)
 */
typedef struct {
 ...
 * @details Thanh ghi chứa giá trị timer (32-bit)
 */
typedef struct {
    __IO uint32_t TMR_VAL;          /**< Timer Value - Giá trị reload cho timer */
} LPIT_TVAL_Type;

/**
 * @brief LPIT Current Timer Value Register (LPIT_CVALn)
 * @details Thanh ghi chứa giá trị hiện tại của timer (read-only)
 */
typedef struct {
    __I  uint32_t TMR_CUR_VAL;      /**< Current Timer Value - Giá trị đếm hiện tại */
} LPIT_CVAL_Type;

/**
 * @brief LPIT Timer Control Register (LPIT_TCTRLn)
 * @details Thanh ghi điều khiển cho mỗi kênh timer
 */
typedef struct {
    __IO uint32_t T_EN       : 1;   /**< Bit 0: Timer Enable */
    __IO uint32_t CHAIN      : 1;   /**< Bit 1: Chain Channel - Nối với kênh trước */
 ...
 * @details Cấu trúc thanh ghi cho một kênh LPIT
 */
typedef struct {
 ...
 * @details Cấu trúc đầy đủ của LPIT module
 */
typedef struct {
...
/** @brief Module Clock Enable - Enable clock cho LPIT module */
...
/** @brief Doze Mode Enable - LPIT hoạt động trong Doze mode */
#define LPIT_MCR_DOZE_EN_MASK   (0x00000004UL)
#define LPIT_MCR_DOZE_EN_SHIFT  (2U)

/** @brief Debug Mode Enable - LPIT hoạt động trong Debug mode */
...
/** @brief Chain Channel - Nối với kênh trước đó */
...
/*******************************************************************************
 * PCC (Peripheral Clock Control) for LPIT
 ******************************************************************************/

/** @brief PCC base address */
#define PCC_BASE            (0x40065000UL)

/** @brief PCC LPIT register offset */
#define PCC_LPIT0_OFFSET    (0xDCU)

/** @brief PCC LPIT register address */
#define PCC_LPIT0           (*((volatile uint32_t *)(PCC_BASE + PCC_LPIT0_OFFSET)))

/** @brief PCC CGC (Clock Gate Control) bit */
#define PCC_CGC_MASK        (0x40000000UL)
#define PCC_CGC_SHIFT       (30U)

/** @brief PCC PCS (Peripheral Clock Source Select) bits */
