...
void Example1_BasicSendReceive(void)
{
    ...
    /* Enable clocks */
    PCC->PCCn[PCC_PORTE_INDEX] = PCC_PCCn_CGC_MASK;     /* Enable PORT E clock */
    PCC->PCCn[PCC_FlexCAN0_INDEX] = PCC_PCCn_CGC_MASK;  /* Enable CAN0 clock */
    
    /* Configure CAN pins */
    PORT_SetPinMux(PORTE, 4U, PORT_MUX_ALT5);  /* CAN0_RX on PTE4 */
    PORT_SetPinMux(PORTE, 5U, PORT_MUX_ALT5);  /* CAN0_TX on PTE5 */
...
void Example2_InterruptMode(void)
{
    ...
    /* Configure CAN pins */
    PORT_SetPinMux(PORTE, 4U, PORT_MUX_ALT5);
    PORT_SetPinMux(PORTE, 5U, PORT_MUX_ALT5);
    
    /* Initialize CAN */
    can_config_t config = {
        .instance = 0,
        .clockSource = CAN_CLK_SRC_SPLLDIV2,
        .baudRate = 500000,
        .mode = CAN_MODE_NORMAL,
        .enableSelfReception = false,
        .useRxFifo = false
    ...
    NVIC_SetPriority(CAN0_ORed_0_15_MB_IRQn, 3);
    NVIC_EnableIRQ(CAN0_ORed_0_15_MB_IRQn);
    NVIC_SetPriority(CAN0_ORed_16_31_MB_IRQn, 3);
